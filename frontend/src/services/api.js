...
// NEW: Shipment Notes (Blockchain)
export const updateShipmentStatusWithNote = async (updateData) => {
  return await axios.post(`${API_URL}/shipments/update-with-note`, updateData, {
    headers: { "Content-Type": "application/json" },
  });
};

export const getShipmentNotes = async (trackingId) => {
  return await axios.get(`${API_URL}/shipments/notes/${trackingId}`);
};
